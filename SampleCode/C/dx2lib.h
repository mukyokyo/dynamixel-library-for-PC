...
// デバイスの種別
typedef enum {
  devtNONE, devtDX, devtAX, devtRX, devtEX, devtMX, devtXL320, devtPRO, devtPROP, devtX, devtY
} TDXL_DevType;
...
DXAPIDLL bool WINAPI DXL_SetMultipleGoalValues(TDeviceID dvid, const uint8_t *ids, const double *values, int num, double sec);
...
DXAPIDLL PDXL_ModelInfo WINAPI DXL_GetModelInfoByModelNo (uint16_t modelno);
...
bool (WINAPI *DXL_SetMultipleGoalValues) (TDeviceID dvid, const uint8_t *ids, const double *values, int num, double sec) = NULL;
...
PDXL_ModelInfo (WINAPI *DXL_GetModelInfoByModelNo) (uint16_t modelno) = NULL;
...
bool LoadDLL (void) {
  ...
  * (FARPROC *)&DXL_SetMultipleGoalValues     = GetProcAddress (hModule, "DXL_SetMultipleGoalValues");
  ...
  * (FARPROC *)&DXL_GetModelInfoByModelNo     = GetProcAddress (hModule, "DXL_GetModelInfoByModelNo");
  ...
  return
    ...
    (DXL_SetMultipleGoalValues     != NULL) &&
    ...
    (DXL_GetModelInfoByModelNo     != NULL);
}
