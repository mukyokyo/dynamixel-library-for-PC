...
const uint32_t baudlist[8] = { 9600, 57600, 115200, 1000000, 2000000, 3000000, 4000000, 4500000 };
const char *opmode[] = {
        ...
        while (kbhit ()) {
          ...
          switch (ch) {
            case 's': // scan
              printf("\nStat scan\n");
              blval = true;
              for (int b = 0; b < 8 && blval; b++) {
                printf("\rhost baudrate:%dbps\n", baudlist[b]);
                DX2_SetBaudrate (dev, baudlist[b]);

                num = 253;
                if (DX2_Ping2 (dev, &num, alm, NULL)) {
                  for (int i = 0; i < num; i++) {
                    printf ("\r id:%3d, device name:%s \n", alm[i].id, DXL_GetModelInfo (dev, alm[i].id)->name);
                  }
                  fflush (stdout);
                }
                if (kbhit ()) {
...
*/
            ...
            case 'b': // change host baudrate
              printf("\nChange host baud\nInput baudrate index\n (0:9600, 1:57600, 2:115200, 3:1M, 4:2M, 5:3M, 6:4M 7:4.5M[bps]) = ");
              if (InputNum (&d)) {
                d = MAX (MIN (d, 7), 0);
              ...
              if (DX2_Ping (dev, id, NULL)) {
                ...
                if (DXL_GetOperatingMode (dev, id, &prevbyteval)) {
                  if ((prevbyteval == 3) || (prevbyteval == 4) || (prevbyteval == 5)) {
                    ...
                    dblval_flg = 1;
                    while (!kbhit ()) {
                      dblval += (dblval_flg * 1.0);
                      if (dblval > 180.0) { dblval_flg = -1; dblval = 180.0; }
                      else if (dblval < -180.0) { dblval_flg = 1; dblval = -180.0; }
